...
/**
 * Telling the estate that a broker has claimed dates.
 *
 * Both channels, as asked: the dashboard bell so it's there when Rodrigo next
 * looks, and email because a 48-hour clock is already running and he may not
 * look today. Neither is allowed to fail the hold itself — the broker has been
 * told the dates are theirs, and that promise doesn't depend on our SMTP.
 */
@Injectable()
export class BrokerNotifyService {
  ...
  private async alert(hold: BrokerHold): Promise<void> {
    try {
      ...
      this.logger.error(`Bell alert failed for hold ${hold.id}: ${String(error)}`);
    }
  }
  ...
  private async email(hold: BrokerHold): Promise<void> {
    ...
    if (!process.env.RESEND_API_KEY || to.length === 0) {
      this.logger.warn(`No email recipients configured — hold ${hold.id} not mailed`);
      return;
    }
    ...
    const html = brandedEmail({
      ...
      body: [
        ...
        emailRows(rows),
        hold.note
          ? emailCopy([emailParagraph(`Their note: “${hold.note}”`, { muted: true, small: true })])
          : '',
        emailCopy([
          emailParagraph(
