 ...
 *
 * Run once from apps/api after the menu_composition migration:
 *
 *   node prisma/scripts/seed-estate-menu.js
 *
 * Plain Node against Prisma Client rather than a .sql file, because the Render
 * shell has no psql and no TypeScript runner — only the things the API itself
 * runs on. Safe to re-run: each dish carries an id derived from its course and
 * name, and an existing row is left exactly as it is. That matters more than
 * it looks — re-running must never wipe the dietary flags the kitchen has since
 * corrected by hand.
 *
...
// [course, category, sortOrder, name, containsShellfish, containsNuts]
const DISHES = [
...
/** Same dish, same id, every run — which is what makes re-running harmless. */
const idFor = (course, name) =>
  `menu_${createHash('md5').update(`${course}|${name}`).digest('hex').slice(0, 20)}`;

async function main() {
  let created = 0;
  let existing = 0;

  for (const [course, category, sortOrder, name, shellfish, nuts] of DISHES) {
    const id = idFor(course, name);
  ...
  console.log(`\nAdded ${created} dishes, ${existing} already there.\n`);
  for (const row of counts.sort((a, b) => (a.course > b.course ? 1 : -1))) {
    console.log(`  ${String(row.course).padEnd(24)} ${row._count._all}`);
  }
  console.log(
    '\nNow set the dietary flags in the dashboard — only nuts and shellfish' +
      '\nnamed in a dish title were filled in here.\n',
  );
}
