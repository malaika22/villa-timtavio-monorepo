...
const defaultDot: Record<ScheduleItem['variant'], string> = {
  default: 'bg-[#6b6560]',
  confirmed: 'bg-[#1e7e34]',
  pending: 'bg-[#d4a373]',
  conflict: 'bg-[#c53030]',
...
export const TodaySchedulePanel = ({ items }: { items: ScheduleItem[] }) => (
  <div>
    <SectionLinkHeader title="Today's Schedule" href="/calendar" linkLabel="Full calendar →" />
        ...
        {items.map((item) => (
          <li key={item.id} className="flex gap-3 px-4 py-3.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-manager-text">{item.title}</p>
