...
const styles: Record<ApprovalQueueStatus, { pill: string; dot: string }> = {
  ...
  'In Progress': { pill: 'bg-manager-info-bg text-[#1e429f]', dot: 'bg-[#1e429f]' },
  Completed: { pill: 'bg-[#f0eeeb] text-manager-text-muted', dot: 'bg-manager-text-muted' },
  Declined: { pill: 'bg-[#f0eeeb] text-manager-text-muted', dot: 'bg-manager-text-muted' },
};

export const ApprovalStatusPill = ({ status }: { status: ApprovalQueueStatus }) => {
